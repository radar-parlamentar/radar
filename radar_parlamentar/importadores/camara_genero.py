...
HEADERS=["codProposicao","txtProposicao","txtTipoProposicao","txtSigla","numNumero","numAno","datApresentacao","codTipoProposicao","codOrgaoOrigem","txtEmenta","txtExplicacaoEmenta","txtIndexacao","codRegime","txtRegime","codApreciacao","txtApreciacao","txtOrgaoOrigem","txtNomeOrgaoOrigem","indGenero","txtNomeAutor","txtSiglaUF","txtSiglaPartido","codPartido","qtdAutores","datDespacho","txtDespacho","codEstadoProposicao","txtEstadoProposicao","codOrgaoEstado","txtSiglaOrgaoEstado","qtdOrgaosComEstado","codProposicaoPrincipal","txtProposicaoPrincipal","ideCadastro","nomeProposicaoOrigem"]

PARTIDOS = {}
DIC_TERMOS = {}
PALAVRAS_MAIS_MAIS = []
DESCARTADAS=['de', 'do', 'da', 'dos', 'das', 'e', 'para', 'com', 'a', 'A']
FILTRADAS=['lei','normas','obrigatoriedade', 'cria\u00e7\u00e3o', 'nacional', 'prazo', 'fixa\u00e7\u00e3o','proibi\u00e7\u00e3o', 'especial', 'pessoa', 'utiliza\u00e7\u00e3o', 'atividade', 'valor', 'institui\u00e7\u00e3o', 'civil', 'estabelecimento', 'registro' ]
LISTA_BASE_PARTIDOS = ['PCB', 'PSD', 'UDN', 'SEM PARTIDO', 'PP', 'PR', 'PTB', 'PRE', 'PRF', 'PST', 'UPF', 'AL', 'FUG', 'PSN', 'PSP', 'PRP', 'PTN', 'PDC', 'PNI', 'PL', 'PPR', 'ARENA', 'PTR', 'PSB', 'PRR', 'PSC', 'PRD', 'LASP', 'PRM', 'PRT', 'PPS', 'PSR', 'PS', 'PDS', 'MTR', 'MDB', 'PMDB', 'PSDB', 'PFL', 'PT', 'PTdoB', 'PDT', 'PJ', 'PCdoB', 'PC', 'PV', 'PRN', 'PPB', 'PSDC', 'PRONA', 'DEM', 'PSOL', 'PMN', 'PSL', 'PRS', 'PRB', 'PE', 'PRC', 'PRL', 'UDB', 'PLC', 'LEC', 'PD', 'ED', 'PRPa', 'PED', 'PNS', 'PPA', 'PNA', 'PSTU', 'PTC', 'PAN', 'PHS', 'PRTB']
...
def parseia_indexacoes(indexacao):
    indexacao1 = [termo.strip() for termo in indexacao.replace('\n','').replace('.','').replace('_','').split(',')]
    indexacao2 = []
...
def parsear_indexacoes_de_proposicoes(lista_proposicoes):
    ...
    for proposicao in lista_proposicoes:
        proposicao['txtIndexacao'] = parseia_indexacoes(proposicao['txtIndexacao'])
        nova_lista_proposicoes.append(proposicao)
        if proposicao['txtSiglaPartido']:
            soma_palavras_no_partido(proposicao['txtSiglaPartido'], proposicao['txtIndexacao'])
...
def principal(fonte=None):
    ...
    retorno = {'partidos': PARTIDOS, 'dic_termos': DIC_TERMOS, 'lista_proposicoes': lista_proposicoes}
