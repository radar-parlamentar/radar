...
class ModelCMSPCase(TestCase):
    ...
    def preencher_banco(casa):
        ...
        partido.save()
        parlamentar = models.Parlamentar(id_parlamentar="1",nome="Teste_vereador")
        parlamentar.save()
        legislatura = models.Legislatura(parlamentar=parlamentar,partido=partido,casa_legislativa=casa)
        legislatura.save()


    import xml.etree.ElementTree as etree
    def test_vereador_sem_partido(self):
        xml_vereador = etree.fromstring("<Vereador Partido=\"nao tem partido\"/>")
        partido = self.xmlCMSP.partido(xml_vereador)
        self.assertEquals(partido,models.Partido.objects.get(nome = models.SEM_PARTIDO))
    ...
    #def test_retorna_vereador_existente(self):
        #TODO: 2 parlamentares identicos podem ser cadastrados se 1 ja existir no banco
        #xml_vereador = etree.fromstring("<Vereador IDParlamentar=\"1\" NomeParlamentar=\"Teste_vereador\"/>") 
        #parlamentar = self.xmlCMSP.votante(xml_vereador)
        #self.assertEquals(parlamentar,models.Parlamentar.objects.get(nome = "Teste_vereador"))

    def test_salva_vereador_inexistente(self):
        xml_vereador = etree.fromstring("<Vereador IDParlamentar=\"999\" NomeParlamentar=\"Nao_consta\"/>")
        parlamentar = self.xmlCMSP.votante(xml_vereador)
        self.assertEquals(parlamentar,models.Parlamentar.objects.get(id_parlamentar = 999))

    def test_salva_legislatura_inexistente(self):
        xml_vereador = etree.fromstring("<Vereador IDParlamentar=\"999\" NomeParlamentar=\"Nao_consta\" Partido=\"PTest\"/>")
        legislatura = self.xmlCMSP.legislatura(xml_vereador)
        self.assertEquals(legislatura,models.Legislatura.objects.get(parlamentar__id_parlamentar="999",partido__nome="PTest"))

    #def test_retorna_legislatura_existente(self):
        #TODO: problema com parlamentares duplicados
        #xml_vereador = etree.fromstring("<Vereador IDParlamentar=\"1\" NomeParlamentar=\"Teste_vereador\" Partido=\"PTest\"/>")
        #legislatura = self.xmlCMSP.legislatura(xml_vereador)
