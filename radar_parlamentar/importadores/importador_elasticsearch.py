"""Módulo responsável por relacionar o radar com o Elasticsearch.

Esse modulo é responsável por transportar as informações que estão no banco de
dados do radar parlamentar para uma instância de Elasticsearch.
É feita a coleta através dos modelos do django, conversão desse modelo para um
objeto RadarParlamentarIndex e, por fim, o envio desse objeto para o
Elasticsearch.
"""
...
ELASTIC_SEARCH_ADDRESS_DEFAULT = {'host': 'elasticsearch', 'port': '8000'}
...
def gerar_json_radar():
    """Construção do modelo json baseado no modelo do banco de dados."""
    list_json = []
...
def conectar_em_elastic_search():
    """Faz a abertura de uma conexão com o ElasticSearch."""
...
def criar_indice(nome_indice):
        ...
        "settings": {
            "analysis": {
                "analyzer": {
                    "my_analyzer": {
                        "tokenizer": "standard",
                        "filter": ["standard", "pt_BR", "lowercase",
                                   "portuguese_stop", "asciifolding"]
                    }
                },
                "filter": {
                    "my_stemmer": {
    ...
    config_mapping = {
        "radar": {
            ...
            "properties": {
                "casa_legilativa_local": {"type": "string"},
                "casa_legislativa_esfera": {"type": "string"},
                "casa_legislativa_id": {"type": "long"},
                "casa_legislativa_nome": {"type": "string"},
                "casa_legislativa_nome_curto": {"type": "string"},
                "proposicao_ano": {"type": "string"},
                "proposicao_data_apresentacao": {
                    "type": "date",
                    "format": "dateOptionalTime"
                },
                "proposicao_descricao": {"type": "string"},
                "proposicao_ementa": {
                    "type": "string",
                    "analyzer": "my_analyzer"
                },
                "proposicao_id": {"type": "long"},
                "proposicao_id_prop": {"type": "string"},
                "proposicao_indexacao": {
                    "type": "string",
                    "analyzer": "my_analyzer"
                },
                "proposicao_numero": {"type": "string"},
                "proposicao_sigla": {"type": "string"},
                "proposicao_situacao": {"type": "string"},
                "votacao_data": {
                    "type": "date",
                    "format": "dateOptionalTime"
                },
                ...
                "votacao_id": {"type": "long"},
                "votacao_id_vot": {"type": "string"},
                "votacao_resultado": {"type": "string"}
            }
        }
    }
    es = conectar_em_elastic_search()
    client_indice = IndicesClient(es)
    client_indice.create(index=nome_indice, body=json.dumps(config_settings))
    client_indice.put_mapping(index=nome_indice, doc_type="radar",
                              body=json.dumps(config_mapping))
