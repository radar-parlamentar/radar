...
def json_proposicao(request, nome_curto_casa_legislativa,
                    identificador_proposicao=None):
    """Retorna o JSON com os dados de todas as votações de uma proposição.
        identificador_proposicao: 'PL-123-2015'
    """
    sigla_proposicao, numero_proposicao, ano_proposicao = \
        identificador_proposicao.split('-')
...
# sigla=sigla_proposicao, numero=numero_proposicao, ano=ano_proposicao)
    proposicoes = \
        models.Proposicao.objects.filter(
            casa_legislativa__nome_curto=nome_curto_casa_legislativa,
            sigla=sigla_proposicao, numero=numero_proposicao,
            ano=ano_proposicao)
    if proposicoes:
