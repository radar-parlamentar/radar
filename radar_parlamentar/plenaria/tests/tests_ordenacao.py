...
class OrdenacaoTest(TestCase):
    ...
    def test_def_ordenar_votantes(self):
        ...
        # QuerySet não está reconhecendo o _set

    """def test_ordem_dos_parl_por_votacao(self):
        proposicao = models.Proposicao.objects.get(id=1)
        votacao =
            Votacao.objects.filter(proposicao=proposicao).order_by('data')
        ...
        lista_ordenada_partidos =
            ordenacao.ordenar_partidos(casa_legislativa, periodo)
        dicionario_votantes = ordenacao.ordenar_votantes(proposicao)
        partidos = ordenacao.ordenar_partidos(casa_legislativa, periodo)
        teste =
            ordenacao.ordem_dos_parl_por_votacao(votacao,dicionario_votantes,partidos)
        print teste"""
